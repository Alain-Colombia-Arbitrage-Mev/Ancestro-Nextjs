...
export default function ChatWidget({ lang }: { lang: string }) {
  ...
  return (
    <>
      ...
      <div className={`chat-widget ${open ? 'chat-widget-open' : ''}`}>
        ...
        {messages.length <= 1 && !loading && (
          <div className="cw-suggestions">
            {suggestions.map((q, i) => (
              <button key={i} className="cw-chip" onClick={() => sendMessage(q)}>{q}</button>
            ))}
          </div>
        )}
      ...
      <style>{`
        .chat-fab{position:fixed;bottom:24px;right:24px;z-index:9998;width:56px;height:56px;border-radius:50%;background:#f8b03b;color:#000;border:none;cursor:pointer;display:flex;align-items:center;justify-content:center;box-shadow:0 4px 20px rgba(248,176,59,0.4),0 2px 8px rgba(0,0,0,0.3);transition:all 0.3s cubic-bezier(0.34,1.56,0.64,1)}
        .chat-fab:hover{transform:scale(1.1);box-shadow:0 6px 28px rgba(248,176,59,0.5)}
        .chat-fab-hidden{opacity:0;transform:scale(0.5);pointer-events:none}
        .chat-backdrop{position:fixed;inset:0;z-index:9998;background:rgba(0,0,0,0.4);backdrop-filter:blur(2px);-webkit-backdrop-filter:blur(2px)}
        ...
        .cw-avatar{width:36px;height:36px;border-radius:10px;background:rgba(248,176,59,0.15);display:flex;align-items:center;justify-content:center}
        ...
        .cw-header-status::before{content:'';width:6px;height:6px;border-radius:50%;background:#4ade80}
        .cw-close{background:none;border:none;color:rgba(255,255,255,0.5);cursor:pointer;padding:6px;border-radius:8px;display:flex;align-items:center;justify-content:center;transition:all 0.2s}
        ...
        .cw-bubble{max-width:82%;padding:12px 16px;border-radius:14px;font-size:14px;line-height:1.55;color:rgba(255,255,255,0.92)}
        .cw-bubble p{margin:0 0 6px}.cw-bubble p:last-child{margin:0}
        ...
        .cw-suggestions{display:flex;flex-wrap:wrap;gap:6px;padding:0 20px 12px}
        .cw-chip{padding:8px 14px;background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.12);border-radius:18px;color:rgba(255,255,255,0.7);font-size:12px;cursor:pointer;transition:all 0.2s;font-family:inherit;text-align:left;line-height:1.3}
        .cw-chip:hover{background:rgba(248,176,59,0.12);border-color:rgba(248,176,59,0.25);color:#f8b03b}
        .cw-input-area{display:flex;align-items:flex-end;gap:10px;padding:14px 20px;border-top:1px solid rgba(255,255,255,0.08);background:rgba(0,0,0,0.3)}
        .cw-input{flex:1;background:rgba(255,255,255,0.06);border:1px solid rgba(255,255,255,0.12);border-radius:12px;padding:11px 14px;color:#fff;font-size:14px;font-family:inherit;resize:none;outline:none;transition:border-color 0.2s;min-height:42px;max-height:100px}
        ...
        .cw-send{width:42px;height:42px;display:flex;align-items:center;justify-content:center;background:#f8b03b;border:none;border-radius:10px;color:#000;cursor:pointer;transition:all 0.2s}
