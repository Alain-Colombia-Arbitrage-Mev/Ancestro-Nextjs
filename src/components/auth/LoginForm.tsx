...
export default function LoginForm({ lang }: LoginFormProps) {
  ...
  const { user, login, loginWithGoogle, setUser, setToken } = useAuth();
  ...
  async function handleNewPasswordSubmit(e: React.FormEvent) {
    ...
    if (newPassword.length < 8) {
      setError(t(lang, 'auth.newPassword.minChars'));
      return;
    }
    if (newPassword !== confirmNewPwd) {
      setError(t(lang, 'auth.newPassword.noMatch'));
      return;
    }
    setIsLoading(true);
    try {
      ...
      <div className="lf-wrapper">
        ...
        <div className="lf-card">
          <div className="lf-header">
            ...
            <h1 className="lf-title">{t(lang, 'auth.newPassword.title')}</h1>
            <p className="lf-subtitle">{t(lang, 'auth.newPassword.subtitle')}</p>
          </div>
          <form onSubmit={handleNewPasswordSubmit} className="lf-form" noValidate>
            <div className="lf-field">
              <label className="lf-label">{t(lang, 'auth.newPassword.newPassword')}</label>
            ...
            <div className="lf-field">
              <label className="lf-label">{t(lang, 'auth.newPassword.confirm')}</label>
              <div className="lf-input-wrap">
                <svg className="lf-input-icon" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                  <rect x="3" y="11" width="18" height="11" rx="2" />
                  <path d="M7 11V7a5 5 0 0 1 10 0v4" />
            ...
            <button type="submit" className="lf-submit" disabled={isLoading}>
              {isLoading ? <span className="lf-spinner" /> : t(lang, 'auth.newPassword.button')}
            </button>
          </form>
        </div>
      ...
      <div className="lf-card">
        ...
        <form onSubmit={handleSubmit} className="lf-form" noValidate>
          ...
          <div className={`lf-field ${fieldErrors.password ? 'lf-field--error' : ''}`}>
            ...
            <div className="lf-input-wrap">
              ...
              <button
                ...
                aria-label={showPassword ? t(lang, 'auth.login.hidePassword') : t(lang, 'auth.login.showPassword')}
                tabIndex={-1}
              >
                {showPassword ? (
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
          ...
          <div className="lf-forgot">
            <Link href={`/${lang}/forgot-password`} className="lf-forgot-link">
              {t(lang, 'auth.login.forgotPassword')}
            </Link>
          </div>
          ...
          >
            {isLoading ? (
              <>
                ...
                {t(lang, 'auth.login.loading')}
              </>
            ) : t(lang, 'auth.login.button')}
          </button>
        </form>

        <div className="lf-divider">
          <span>{t(lang, 'auth.login.orContinueWith')}</span>
        </div>
        ...
        >
          ...
          {!isGoogleLoading && t(lang, 'auth.login.google')}
        </button>

        <p className="lf-register-row">
          {t(lang, 'auth.login.noAccount')}{' '}
          <Link href={`/${lang}/register`} className="lf-register-link">
